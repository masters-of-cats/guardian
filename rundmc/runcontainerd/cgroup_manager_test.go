...
var _ = Describe("CgroupManager", func() {
	...
	Describe("SetUseMemoryHierarchy", func() {
		var (
			...
			containerStateFileContent string
			returnedErr               error
		)
		...
		BeforeEach(func() {
			...
			containerStateFileContent = fmt.Sprintf(`{
				"cgroup_paths": {
					"memory": "%s"
				}
			}`, memoryDir)
		...
		Context("when the state dir for the container doesn't exist", func() {
			...
			It("returns an error", func() {
				Expect(returnedErr).To(MatchError(ContainSubstring("no such file or directory")))
			})
		})

		Context("when the container state file is malformed", func() {
			BeforeEach(func() {
				containerStateFileContent = ""
			})
