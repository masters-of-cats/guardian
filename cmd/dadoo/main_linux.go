...
func run() int {
	...
	runcRoot := flag.String("runc-root", "", "path to runc root dir")
	flag.Parse()
	...
	if *tty {
		...
		runcExecCmd = dadoo.BuildRuncCommand(runtime, runMode, processStateDir, containerId, ttySocketPath, logFile, *runcRoot)
	} else {
		runcExecCmd = dadoo.BuildRuncCommand(runtime, runMode, processStateDir, containerId, "", logFile, *runcRoot)
