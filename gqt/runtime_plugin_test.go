	...
	Context("when a runtime plugin is provided", func() {
		...
		Describe("creating a container", func() {
			...
			Describe("creating a pea", func() {
				FIt("executes the plugin, passing the correct args for whatever a pea needs", func() {
					...
					// TODO WHY WHY BUT DOES THIS NOT WORK?
					// ioutil.ReadDir(client.TmpDir)
					...
					// TODO WHY BUT ITS LINUX
					// cmd := exec.Command("ls", "-R", client.TmpDir)
					// cmd.Stdout = GinkgoWriter
					// cmd.Stderr = GinkgoWriter
					// Expect(cmd.Run()).To(Succeed())
